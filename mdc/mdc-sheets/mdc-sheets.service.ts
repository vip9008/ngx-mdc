...
import { MdcHostDirective } from '../mdc-common/mdc-host.directive';
...
export class MdcSheetsService {
    ...
    get rootViewContainerRef(): ViewContainerRef {
        ...
        const mdcHost: MdcHostDirective = appInstance.mdcComponentContainer;
        ...
        return mdcHost.viewContainerRef;
    }
    ...
    public createSideSheet<T = any>(dynamicComponent: any, type: 'above-content' | 'coplanar' | 'modal' = 'above-content', data: Object = {}, mdcHost: MdcHostDirective = null): ComponentRef<MdcSideSheetComponent<T>> | null {
        if (this.currentSideSheet) {
            return null;
        }

        const vcr: ViewContainerRef = mdcHost ? mdcHost.viewContainerRef : this.rootViewContainerRef;

        const componentRef: ComponentRef<T> = vcr.createComponent(dynamicComponent) as ComponentRef<T>;
        Object.assign(componentRef.instance, data);

        const sheetRef: ComponentRef<MdcSideSheetComponent<T>> = vcr.createComponent(MdcSideSheetComponent, {
            projectableNodes: [
